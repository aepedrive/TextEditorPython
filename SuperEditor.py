...
def update_line_number(event=None):
    ...
    curline, curcolumn = textPad.index('insert').split('.')
    infobar.config(text='Line: %s | Column: %s' %(curline, curcolumn))
...
def undo_highlight():
    textPad.tag_remove("active line", 1.0, "end")
...
def open_file(event=None):
    ...
    if filename == "": #If no file chosen
        ...
        root.title(os.path.basename(filename) + " - pyPad") #Returning the basename if file
        textPad.delete(1.0, END)
        fh = open(filename, "r")
...
#override close button and redirect to exit_editor
...
#Creating Info Bar // widget within text budget
...
textPad.bind("<Button-1>", update_line_number)#BUG, displays innacurate line and column numbers
textPad.tag_configure("active_line", background="red")
